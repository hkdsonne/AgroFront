...
//
//        try {
//            if (!courierId || isNaN(courierId) || courierId <= 0) {
...
//
//        const courierId = document.getElementById('completeCourierId').value;
//        const orderId = document.getElementById('completeOrderId').value;
//        const completeTime = document.getElementById('completeTime').value;
//
...
//            const formattedTime = new Date(completeTime).toISOString();
//
...
//                <p>Курьер: ${data.courier_id}</p>
//                <p>Время завершения: ${new Date(data.complete_time).toLocaleString()}</p>
//            `;
//
//        } catch (error) {
...
//            completeResponse.innerHTML = `Ошибка: ${error.message}`;
//
//        } finally {
//            submitBtn.disabled = false;
