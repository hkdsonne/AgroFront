...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadCustomerOrders() {
        ...
        .catch(error => {
            console.error('Ошибка загрузки заказов:', error);
            document.getElementById('ordersContainer').innerHTML = `
                <div class="error-message">Ошибка загрузки заказов: ${error.message}</div>
            `;
        });
    }
    ...
    function renderOrders(orders) {
        ...
        orders.forEach(function(order) {
            ...
            orderCard.innerHTML = `
                <div class="order-header">
                    <div class="order-id">Заказ #${order.order_id || order.id}</div>
                ...
                <div class="order-details">
                    ...
                    <div>
                        <div class="form-label">Время доставки</div>
                        <div>${order.delivery_hours ? order.delivery_hours.join(', ') : ''}</div>
                    </div>
                </div>
                ${order.courier_id ? `<div><strong>Курьер:</strong> #${order.courier_id}</div>` : ''}
            `;
